import React from 'react';

const Groups = () => {
  return (
    <div className="Groups">
      < div >
        <body >

        <div className = "Headergroup" >
        <p className = "desplitgroup" > DSplit </p> </div>

        <div className = "bodygroup" >

        <details>
          <summary>
        <div className = "firstheadergroup" >
        <p className = "fhg" > Group: 1</p> 
        <p className = "fhg" > GroupTransactions </p>

        </div>
          </summary>

        <div className = "addressesgroup" >
        <div className = "a1" >
        <div className = "left" >
        <p className = "address" > 0xd35781d9712436ad6dcad30a51e408d4232781ef7296d9a8d53f4c7203744809 </p> <div className = "slash" > - </div> 
        <p className = "value" > 1.000, 29 $ </p> </div>

        <div className = "right" >
        <p className = "accept" > Accept </p> <p className = "slash" >
        </p> <p className = "reject" > Reject </p> </div> </div> 
        <div className = "a1" >
        <div className = "left" >
        <p className = "address" > 0xd35781d9712436ad6dcad30a51e408d4232781ef7296d9a8d53f4c7203744809 </p>
        <div className = "slash" > - </div> <p className = "value" > 1.000, 29 $ </p> </div>

        <div className = "right" >
        <p className = "accept" > Accept </p> <p className = "slash" > | </p> <p className = "reject" > Reject </p> </div> </div> <div className = "a1" >
        <div className = "left" >
        <p className = "address" > 0xd35781d9712436ad6dcad30a51e408d4232781ef7296d9a8d53f4c7203744809 </p> 
        <div className = "slash" > - </div> <p className = "value" > 1.000, 29 $ </p> </div>

        <div className = "right" >
        <p className = "accept" > Accept </p> <p className = "slash" > | </p> <p className = "reject" > Reject </p> </div> </div>
        </div> 
        </details>
        

        <details>
          <summary>
        <div className = "firstheadergroup" >
        <p className = "fhg" > Group: 2</p> 
        <p className = "fhg" > GroupTransactions </p>

        </div>
          </summary>

        <div className = "addressesgroup" >
        <div className = "a1" >
        <div className = "left" >
        <p className = "address" > 0xd35781d9712436ad6dcad30a51e408d4232781ef7296d9a8d53f4c7203744809 </p> <div className = "slash" > - </div> 
        <p className = "value" > 1.000, 29 $ </p> </div>

        <div className = "right" >
        <p className = "accept" > Accept </p> <p className = "slash" >
        </p> <p className = "reject" > Reject </p> </div> </div> 
        <div className = "a1" >
        <div className = "left" >
        <p className = "address" > 0xd35781d9712436ad6dcad30a51e408d4232781ef7296d9a8d53f4c7203744809 </p>
        <div className = "slash" > - </div> <p className = "value" > 1.000, 29 $ </p> </div>

        <div className = "right" >
        <p className = "accept" > Accept </p> <p className = "slash" > | </p> <p className = "reject" > Reject </p> </div> </div> <div className = "a1" >
        <div className = "left" >
        <p className = "address" > 0xd35781d9712436ad6dcad30a51e408d4232781ef7296d9a8d53f4c7203744809 </p> 
        <div className = "slash" > - </div> <p className = "value" > 1.000, 29 $ </p> </div>

        <div className = "right" >
        <p className = "accept" > Accept </p> <p className = "slash" > | </p> <p className = "reject" > Reject </p> </div> </div>
        </div>  
        </details>
   
        
        <details>
          <summary>
        <div className = "firstheadergroup" >
        <p className = "fhg" > Group: 3</p> 
        <p className = "fhg" > GroupTransactions </p>

        </div>
          </summary>

        <div className = "addressesgroup" >
        <div className = "a1" >
        <div className = "left" >
        <p className = "address" > 0xd35781d9712436ad6dcad30a51e408d4232781ef7296d9a8d53f4c7203744809 </p> <div className = "slash" > - </div> 
        <p className = "value" > 1.000, 29 $ </p> </div>

        <div className = "right" >
        <p className = "accept" > Accept </p> <p className = "slash" >
        </p> <p className = "reject" > Reject </p> </div> </div> 
        <div className = "a1" >
        <div className = "left" >
        <p className = "address" > 0xd35781d9712436ad6dcad30a51e408d4232781ef7296d9a8d53f4c7203744809 </p>
        <div className = "slash" > - </div> <p className = "value" > 1.000, 29 $ </p> </div>

        <div className = "right" >
        <p className = "accept" > Accept </p> <p className = "slash" > | </p> <p className = "reject" > Reject </p> </div> </div> <div className = "a1" >
        <div className = "left" >
        <p className = "address" > 0xd35781d9712436ad6dcad30a51e408d4232781ef7296d9a8d53f4c7203744809 </p> 
        <div className = "slash" > - </div> <p className = "value" > 1.000, 29 $ </p> </div>

        <div className = "right" >
        <p className = "accept" > Accept </p> <p className = "slash" > | </p> <p className = "reject" > Reject </p> </div> </div>
        </div> 
        </details>
        </div>


        
        </body> </div>
    </div >
  );
};


export default Groups;